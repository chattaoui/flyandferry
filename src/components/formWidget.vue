<template>
  <div
    class="travel-booking"
    :style="
      !currentMenu ? 'top:66.4vh;min-height:0px' : 'top:60vh;height:55rem'
    "
  >
    <svg
      style="position: absolute; top: 1.3rem; right: 2rem; cursor: pointer"
    ...
    <div class="options-container flexContainer">
      <div style="white-space: nowrap">
        <button
          class="menuButton"
          :class="{ 'background-focus': currentMenu === 'trip' }"
        ...
        <button
          class="menuButton"
          :class="{ 'background-focus': currentMenu === 'date' }"
          @click="showMenu('date')"
        >
          Select Date
        </button>
        <button
          class="menuButton"
          :class="{ 'background-focus': currentMenu === 'passengers' }"
        ...
        <button
          class="menuButton"
          :class="{ 'background-focus': currentMenu === 'vehicles' }"
    ...
    <div v-if="currentMenu === 'trip'" class="menu">
      <div class="menu-column">
        ...
        <ul
          v-for="(port, index) in Object.keys(filteredRouteList)"
          :key="`DepartPort_` + index"
        >
          <li
            @click="selectPortItem(index, port)"
            class="port-name"
      ...
      <div class="menu-column">
        ...
        <ul
          ...
          v-if="depPort.length > 0"
          v-for="(country, indexx) in Object.keys(filteredRouteList[depPort])"
          :key="`depPort_` + indexx"
        >
          <h3 style="text-align: start">{{ countryCodes[country] }}</h3>
          <li
            @click="handleRouteClick(port, index, indexx)"
            v-for="(port, index) in filteredRouteList[depPort][country]"
            :key="port + index"
            class="port-name"
            :class="{
      ...
      <div class="map-column">
        <MapView
          ...
          :place="depPort.length !== `` ? `${depPort.toString()}` : null"
          :destination="destPort !== `` ? destPort : null"
        />
      </div>
    </div>
    <div v-if="currentMenu === 'date'" class="menu">
      ...
      <VueCtkDateTimePicker
        ...
        v-model="departDate"
        format="YYYY-MM-DD"
        :enabledDates="datestoHighlight"
        ref="dateTimePicker"
        :range="tripType === `oneway` ? false : true"
        :noShortcuts="true"
        :no-header="true"
        :inline="true"
        :only-date="true"
    ...
    >
      <div class="category-container" v-if="categoryExists('Adult')">
        ...
        >
          ...
          <path
            class="st0"
            d="M332.7,281.9l-32.2-36.7C300.5,245.1,324.1,256.3,332.7,281.9z"
          />
        </svg>
        <span style="color: black !important">Adults</span>
        <div class="number-input">
          <button type="button" @click="adults > 1 ? adults-- : null">-</button>
          <input
            type="number"
            ref="adult"
            v-model="adults"
    ...
    <div
      v-if="currentMenu === 'vehicles'"
      class="menu"
      style="flex-direction: column; gap: 3rem; align-items: center"
    >
      <div v-if="!selectingTrailer">
        ...
        <div
          style="display: flex; justify-content: flex-start; margin-top: 1em"
        >
          <p style="color: #ffffff">besoin d'une remorque ?</p>
          <label class="custom-checkbox">
            <input
              v-model="TrailerIsSelected"
              @change="showTrailerOptions()"
              type="checkbox"
            />
        ...
        <div>
          <div class="radio-section">
            <div class="radio-list">
              <div
                class="radio-item"
                :style="{ width: brandListWidth }"
                v-for="(vehicleBrand, index) in searchCar()"
                :key="vehicleBrand + `_item`"
              >
                <input
                  name="car-brand"
                  type="radio"
                  :id="vehicleBrand + `_item`"
            ...
            <div
              v-if="selectedcarBrand !== ``"
              class="radio-list"
              style="transform: scale(0.8); margin-right: -2rem"
            >
              <div
                class="radio-item car-models"
                v-for="(vehicleModel, index) in getCarModel()"
                :key="vehicleModel['Model'] + `_item`"
              >
                <input
                  name="car-model"
                  type="radio"
                  :id="vehicleModel['Model'] + `_item`"
                  v-model="selectedcarModel"
                  :value="vehicleModel"
                />
                <label
                  style="white-space: nowrap"
                  :for="vehicleModel['Model'] + `_item`"
          ...
          <div class="radio-list">
            ...
            >
              <input
                name="trailer"
                type="radio"
                @change="testingtrailer(trailer)"
                :id="trailer.Description"
                v-model="selectedTrailer"
                :value="trailer"
              />
              <label :for="trailer.Description">{{
                TrailerLabelEdit(trailer.Description)
              }}</label>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async getTimeTable(fromDate, toDate) {
      ...
      if (this.selectedTrailer.length)
    ...
    async useGetScheduleByLine(LineCode) {
      try {
        const response = await this.$axios.post(
          "https://cms.4help.tn/api/GNV_API/getScheduleByLine",
          { LineCode }
        );
        return response.data;
      } catch (error) {
        console.error("Error in useGetScheduleByLine:", error);
    ...
    async getVehiculesPassengers() {
      ...
      this.VehiclePassengers = await this.$axios.request(config).then((res) => {
        return res.data;
      });
    ...
    async getRoutes() {
      ...
      this.gnvRoutes = await this.$axios
        .get("https://cms.4help.tn/api/GNV_API/getTables")
        .then((res) => {
          return res.data.Routes.Route;
        });
    },
    showMenu(menu) {
    ...
    getDepartDest(ROUTES) {
      ...
      ROUTES.forEach((route) => {
        ...
        const departPortName = hasDollarProp
          ? route["$"].DepartPortName
          : route.DepartPortName;
        const destinationPortName = hasDollarProp
          ? route["$"].DestinationPortName
          : route.DestinationPortName;
        const departPort = hasDollarProp
          ? route["$"].DepartPort
          : route.DeparturePortCode;
        const destinationPort = hasDollarProp
          ? route["$"].DestinationPort
          : route.ArrivalPortCode;
        const destinationPortCountry = hasDollarProp
          ? route["$"].DestinationPortCountry
          : route.DestinationPortCountry;

        const updatePortNameCode = (portName, portCode, codeType) => {
          if (this.portNameCode.hasOwnProperty(portName)) {
            // If the current value is not an object, convert it to an object with the existing value under 'CTN'
            if (typeof this.portNameCode[portName] === "string") {
              const existingPortCode = this.portNameCode[portName];
              this.portNameCode[portName] = { CTN: existingPortCode };
            }
            // Add the new port code under the specified code type if it doesn't already exist
            if (!this.portNameCode[portName][codeType]) {
              this.portNameCode[portName][codeType] = portCode;
            }
          } else {
            // If the property doesn't exist, create a new object with the new port code
            this.portNameCode[portName] = { [codeType]: portCode };
          }
        };

        // Refactored usage with the new updatePortNameCode function
        updatePortNameCode(
          departPortName,
          departPort,
          hasDollarProp ? "CTN" : "GNV"
        );
        updatePortNameCode(
          destinationPortName,
          destinationPort,
        ...
        if (
          !this.filteredRouteList[departPortName].hasOwnProperty(
        ...
        if (
          !this.filteredRouteList[departPortName][
            destinationPortCountry
          ].includes(destinationPortName)
        )
          this.filteredRouteList[departPortName][destinationPortCountry].push(
            destinationPortName
          );
      });
    ...
    getCurrentAndLastDayOfNextMonth() {
      ...
      const formattedLastDayOfNextMonth = formatDate(
        nextMonthYear,
        nextMonth,
        lastDay
      );
    ...
    async getCountryNameFromCity(cityName) {
      ...
      const frenchPlaceName = await this.translateCityName(
        cityName,
        "it",
        "fr"
      );
      ...
      try {
        ...
        if (response.data && response.data.data) {
          const frenchCountryName = await this.translateCityName(
            response.data.data.country,
            "en",
            "fr"
          );

          const countryCCA3 = await this.$axios
            .get(
              `https://restcountries.com/v3.1/name/${response.data.data.country}`
            )
            .then((res) => {
              return res.data[0].cca3;
            });

          await this.$axios.post("https://cms.4help.tn/api/GNV_API/places", {
            placeName: cityName,
            frenchPlaceName: frenchPlaceName,
            frenchCountryName: frenchCountryName,
        ...
        try {
          ...
          const newName = await this.$axios
            .get(
              `http://api.geonames.org/searchJSON?q=${frenchPlaceName}&maxRows=1&lang=fr&username=ahmed_flyandferry`
            )
            .then((res) => {
              return res.data.geonames[0].countryName;
            });

          const engCountryName = await this.translateCityName(
            newName,
            "fr",
            "en"
          );
          ...
          const countryCCA3 = await this.$axios
            .get(`https://restcountries.com/v3.1/name/${engCountryName}`)
            .then((res) => {
              return res.data[0].cca3;
            });

          const newFrenchCountryName = await this.translateCityName(
            newName,
            "en",
            "fr"
          );
          ...
          await this.$axios.post("https://cms.4help.tn/api/GNV_API/places", {
            placeName: cityName,
            frenchPlaceName: frenchPlaceName,
            frenchCountryName: newFrenchCountryName,
            countryCCA3: countryCCA3,
          });
    ...
    async getCityAndCountryName(cityName) {
      try {
        const frenchNames = this.oldPlaces[cityName]
          ? this.oldPlaces[cityName]
          : await this.getCountryNameFromCity(cityName);
    ...
    async filterGNVRoutes() {
      const updatePromises = this.gnvRoutes.map(async (route) => {
        const departPortInfo = await this.getCityAndCountryName(
          route.DeparturePortDescription
        );
        const arrivalPortInfo = await this.getCityAndCountryName(
          route.ArrivalPortDescription
        );
  ...
  watch: {
    ...
    async depPort(value) {
      ...
      const Provider = this.portNameCode[value];
      console.log(monthDates);
      if (Object.keys(Provider).includes("CTN")) {
        const code = Provider["CTN"];
        console.log(code);
        const destPorts = this.Routes.filter(
          (obj) => obj["$"].DepartPort === code
        ...
        console.log(dates);
        this.datestoHighlight = dates
          .filter((date) => !!date)
          .flatMap((date) =>
            date.map((obj) => obj.DepartDateTime.split("T")[0])
          );
