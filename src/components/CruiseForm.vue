      ...
      <form class="search-form" method="POST">
        ...
        <div class="formbold-form-step-2">
          ...
          <div>
            ...
            <input type="text" name="address" id="address" placeholder="Flat 4, 24 Castle Street, Perth, PH1 3JY"
              class="formbold-form-input" />
          </div>
        </div>

        <div class="formbold-form-step-1 test-center active" style="flex-direction: column; gap: 30px; margin-top: 35px">
          <div class="group">
            ...
            <input @input="filterRoutes" style="font-size: 13px;" placeholder="Entrez un port, un pays ou un opérateur"
              id="searchRef" ref="searchRef" v-model="searchInp" type="search"
              @focus="searchFocused = true; showPopular = true" class="input">
          ...
          <a class="formbold-btn" v-if="!searchFocused" @click="handleFormClick">
            <span></span>
            <span></span>
            <span></span>
            <span></span>
          ...
          <div class="search-menu" v-if="showPopular">
            ...
            <div v-if="showRoutes">
              <div>
                <h5 style="color:azure;font-size: 16px;">{{ language.form.Routes }}</h5>
              </div>
            ...
            <div>
              <h5 style="color:azure;font-size: 16px;">{{ language.form.popRoutes }}</h5>
            </div>
            <div class="popular-ferry-list">
              <div class="list-item" v-for="(trip, index) in popularTrips" :key="index">
                <div class="departure">
...
export default {
  ...
  methods: {
    filterRoutes() {
      this.searchedRoutes = this.popularTrips.map((trip) => {
        if (Object.values(trip).some((value) => value.toUpperCase().includes(this.searchInp.toUpperCase()))) {
          return trip;
        }
        return null;
      }).filter(Boolean)
      this.searchInp !== "" && this.searchedRoutes.length > 0 ? this.showRoutes = true : this.showRoutes = false
    },
    handleFormClick() {
...
.formbold-form-wrapper {
  ...
  background: linear-gradient(to top right, rgba(228, 85, 41, 0.7), rgba(26, 142, 153, 0.9));
